...
SCEN<-"GrOn"
climate<-"current"
scen_path<-paste0("Y:/duveneck/LANDIS_OUTPUTS/PNET_178_scenario_",SCEN,"_",climate,"/replicate",scen_rep,"/output/agbiomass/")
setwd(scen_path)
...
species <- as.character(species_file$GENUSPEC)
spp_group <- as.character(species_file$spp_group)
species_group <- as.data.frame(cbind(species, spp_group))
colnames(species_group)<-c("SPCD","SPG")
USG<-as.character(unique(species_group$SPG))#[-species_group$SPG=="southern_pine"]
USG<-USG[USG!="southern_pine"]
...
map.forest.type <- function(SCEN = "GrOn", scen_rep = 20170110901, climate = "current"){
  
  # Set working directory to correct scenario/rep directory
  scen_path <- paste0("Y:/duveneck/LANDIS_OUTPUTS/PNET_178_scenario_",SCEN,"_",climate,"/replicate",scen_rep,"/output/agbiomass/")
  setwd(scen_path)
  
  # Get raster extent and projection from eco map
  eco_map <- raster("Y:/duveneck/LANDIS_INPUTS/input_rasters/eco_11_17_2016B.img")
  PRJ <- projection(eco_map)
  EXT <- extent(eco_map)
  eco1 <- eco_map > 0
  eco_df <- as.data.frame(eco1)
  
  # Create df of species
  species_file <- read.csv("Y:/duveneck/LANDIS_INPUTS/eco_spp_inc/LANDIS_NE_SPECIES.csv")
  species <- as.character(species_file$GENUSPEC)
  spp_group <- as.character(species_file$spp_group)
  species_group <- as.data.frame(cbind(species, spp_group), stringsAsFactors = FALSE)
  colnames(species_group) <- c("SPCD","SPG")
  
  # Get unique spp groups except southern pine (zero frequency)
  USG <- unique(species_group$SPG)
  USG <- USG[USG != "southern_pine"]
  USG_short <- c("SF", "NH", "AB", "TC", "PE", "OK", "NA")   # for frequency table labels
  ...
  for (YR in years){
    ...
    spg_matrix <- matrix(nrow = nrow(eco1) * ncol(eco1), ncol = length(USG) + 1)
    for (SG in 1:length(USG)){   # For each unique species group.
      sg <- USG[SG]  # Unique species group name
      print(sg)
      spp <- subset(x = species_group, subset = species_group$SPG %in% sg)$SPCD  #species in the group
      print(spp)
      spp_stack <- sum(stack(paste0(spp,"/AGBiomass_",YR,".img")))  
      spg_matrix[,SG] <- as.data.frame(spp_stack)[,1]
    }   # End of species group loop
    spg_matrix[,SG+1]<-eco_df[,1]  #place ecoregion forested mask on end of matrix to make type "7" to be no data.
    FT_data_frame<-max.col(spg_matrix, ties.method = c("last"))#which forest type.
    FT_matrix<-matrix(FT_data_frame, nrow=nrow(eco_map),ncol=ncol(eco_map), byrow=T)
    FT_raster<-raster(FT_matrix)  
